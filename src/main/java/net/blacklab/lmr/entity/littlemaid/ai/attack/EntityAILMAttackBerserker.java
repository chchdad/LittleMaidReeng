...
/**
 * 狂战士独立 AI (原生专属 setSwing 动画 + 真红温过载版)
 */
	...
	private void checkKillExtension() {
		if (this.entityTarget != null && (this.entityTarget.isDead || this.entityTarget.getHealth() <= 0.0F)) {
			if (this.isFrenzy && !this.killClaimed) {
				...
				setMaidOverDrive(this.frenzyTimer);
				System.err.println("[LMR-BERSERKER] 🩸 杀戮续航触发！红色过载充能延长！");
			}
		}
	}
		...
		if (this.lastTickHealth > currentHp) {
			...
			if (this.isFrenzy) {
				...
				if (this.lastTickHealth > maxHp * 0.5F && currentHp <= maxHp * 0.5F && currentHp > 0.0F) {
					...
					setMaidOverDrive(140); 
					System.err.println("[LMR-BERSERKER] 🩸 绝境爆发！反射真红温注入成功！");
				}
			}
		}
		...
		if (actionDelayTimer > 0) {
			...
			if (pendingDash) {
				...
				if (actionDelayTimer <= 0) {
					...
					// 抛弃原版 swingArm 强行干预，仅用原生 setSwing
					theMaid.setSwing(10, EnumSound.ATTACK, false);
